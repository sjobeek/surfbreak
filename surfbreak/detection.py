...
__all__ = ['dense_optical_flow', 'heatmap_fit', 'trim_image', 'avg_wave_flows', 'detect_surfzone']
...
def detect_surfzone(video_file, n_samples=10, processes=4, draw_fit=False):
    """Detects the region of the image which contains breaking waves.
    ...
    with multiprocessing.Pool(processes=processes) as pool:
        ...
        start_times = (sample_spacing * i for i in range(n_samples))
        args = ((video_file, st) for st in start_times)

        avg_flows = pool.starmap(avg_wave_flows, args)
